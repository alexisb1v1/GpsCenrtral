...
export class DriverApiService {
  ...
  async getAll(tenantId?: string): Promise<ApiResponseDto<DriverDto[]>> {
    const queryParams = tenantId ? `?tenantId=${tenantId}` : '';
    const response = await fetch(`${API_CONFIG.BASE_URL}/drivers${queryParams}`, {
      method: 'GET',
      headers: this.getAuthHeaders(),
      cache: 'no-store',
    });
    return this.handleResponse<DriverDto[]>(response);
  }

  async getById(id: string): Promise<ApiResponseDto<DriverDto>> {
  ...
  async create(data: CreateDriverDto): Promise<ApiResponseDto<DriverDto>> {
    const response = await fetch(`${API_CONFIG.BASE_URL}/drivers/create`, {
      method: 'POST',
      headers: this.getAuthHeaders(),
  ...
  async update(id: string, data: UpdateDriverDto): Promise<ApiResponseDto<DriverDto>> {
    const response = await fetch(`${API_CONFIG.BASE_URL}/drivers/${id}`, {
      method: 'PUT',
      headers: this.getAuthHeaders(),
      body: JSON.stringify(data),
